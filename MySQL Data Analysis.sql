...
create view Amount_Recived as
select c.customerNumber,c.customerName ,c.salesRepEmployeeNumber as Sales_Representative,
c.country,sum(p.amount) as Total_Amount
from customers c left join 
payments p on p.customerNumber = c.customerNumber
group by c.customerNumber,c.customerName;

create view Price_Generated as
select o.customerNumber, sum(od.quantityOrdered * priceEach) as Total_Amt from orderdetails od
inner join orders o on od.orderNumber = o.orderNumber
group by o.customerNumber;
-- -- -- -- 

delimiter $
create procedure Cutomer_Payment_Status(Customer_Name varchar(150))
begin
select pg.customerNumber,ar.customerName,ar.country, ar.Sales_Representative ,
pg.Total_Amt as Total_Bill ,
ar.Total_Amount as Amount_Received ,
case 
	when
        ar.Total_Amount < pg.Total_Amt then 'Payment Pending'
        else 'All Payment Recived'
        end as Payment_Status,
sum(pg.Total_Amt - ar.Total_Amount ) as Pending_Amount        
from Amount_Recived ar
inner join Price_Generated pg on pg.customerNumber = ar.customerNumber
where ar.customerName = Customer_Name
group by pg.customerNumber,ar.customerName;
end $
delimiter ;

call Cutomer_Payment_Status('Euro+ Shopping Channel');
